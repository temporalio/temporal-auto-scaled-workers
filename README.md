...
| `scale_up_dispatch_rate_epsilon` | `0` | Relative band (a fraction of the dispatch rate, capped at `0.10`) for the dispatch-rate-within-epsilon detector; `0` disables it. When set, a queue type's scale-up is suppressed while its dispatch rate holds within this band under a backlog above the scale-up threshold. |

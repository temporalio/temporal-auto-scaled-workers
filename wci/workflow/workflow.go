...
func (d *WorkflowRunner) run(ctx workflow.Context) error {
	...
	if d.reliablePollCadence {
		for !d.shouldContinueAsNew(ctx) {
			for i := 0; i < tasBatchSizePerLoopRun; i++ {
				// Pull at most one freshly-arrived signal into the queue per processed one, so
				// intake keeps pace with processing instead of front-loading the whole channel
				// (which could overflow the queue cap and drop signals).
				if d.signalHandler.signalSelector.HasPending() {
					d.signalHandler.signalSelector.Select(ctx)
				}
				if !d.processNextQueuedTaskAddSignal(ctx) {
					break
				}
			}

...
// Per-queue detector state keys (prefix = queue-type name).
...
func TestNoSyncValidateConfig(t *testing.T) {
	...
	t.Run("epsilon>0 with confirm window <= 0 rejected", func(t *testing.T) {
		cfg := iface.ScalingAlgorithmConfig{
			...
			configNoSyncScaleUpDispatchRateEpsilonConfirmMsKey: int64(0),
		}
		require.Error(t, a.ValidateConfig(ctx, cfg))
	})
	...
	t.Run("epsilon>0 with confirm window <= poll interval rejected", func(t *testing.T) {
		...
		cfg := iface.ScalingAlgorithmConfig{
			configNoSyncScaleUpDispatchRateEpsilonKey:          float64(0.08),
			configNoSyncScaleUpDispatchRateEpsilonConfirmMsKey: int64(60_000),
...
func TestNoSyncProcessMetricsPoll(t *testing.T) {
	...
	t.Run("dispatch-rate-within-epsilon detection never gates the lifetime-refresh (maintenance) path", func(t *testing.T) {
		// The detector is driven into an ACTIVE suppressing state (backlog above the scale-up threshold, within band past the confirm
		// window) and the last scale-up predates the lifetime. Growth is gated, but maintenance (lifetime
		// refresh) must still fire -- an expired worker is replaced even while suppressing.
		now := time.Now().UnixMilli()
		state := iface.ScalingAlgorithmStatus{
			stateLastScaleUpTimestampKey:                                       now - 10_000, // older than the 1s lifetime
			fmt.Sprintf(stateDispatchRateWithinEpsilonSinceKeyFmt, "workflow"): now - 46_000, // within band past the 45s confirm window
		...
		cfg := iface.ScalingAlgorithmConfig{
			...
			configNoSyncScaleUpBacklogThresholdKey:             int64(0),     // any backlog is above the scale-up threshold
...
// Covers the epsilon>0 detector end to end: a per-type table runs it against workflow, activity, and
// nexus (asserting each queue's own state keys); cross-queue cases then exercise interactions no single type can.
func TestNoSyncDispatchRateWithinEpsilonDetection(t *testing.T) {
	...
	for _, q := range queues {
		...
		t.Run(q.name, func(t *testing.T) {
			t.Run("first in-band poll anchors and persists; growth still fires", func(t *testing.T) {
				now := time.Now().UnixMilli()
				...
				assert.Contains(t, r.Status, kWithinEpsilon)
				assert.Contains(t, r.Status, kRef)
				assert.Contains(t, r.Status, kSuppress)
				assert.NotEqualValues(t, 0, r.Status[kWithinEpsilon], "confirm timer anchored")
			...
			t.Run("still within band past the confirm window -> suppress; fast path obeys; poll backs off", func(t *testing.T) {
				cfg := active()
				now := time.Now().UnixMilli()
				state := iface.ScalingAlgorithmStatus{
					stateLastScaleUpTimestampKey: now,          // recent -> poll sees no maintenance
					kWithinEpsilon:               now - 46_000, // began before the 45s confirm window
					kRef:                         float64(5),
				}
				...
				require.NoError(t, err)
				assert.Empty(t, fr.Actions, "fast-path growth suppressed while within band")
				assert.Equal(t, 3, fr.ThrottledCount)
			})

			t.Run("suppression lease renews on a sustained in-band rate", func(t *testing.T) {
				now := time.Now().UnixMilli()
			...
			t.Run("reference anchored once; drift past the fixed band resumes", func(t *testing.T) {
				...
				require.NoError(t, err)
				assert.EqualValues(t, 100, r1.Status[kRef], "anchored at the first in-band rate")
				anchored := r1.Status[kWithinEpsilon]

				r2, err := a.ProcessMetricsPoll(ctx, cfg, r1.Status, q.snap(100, 105)) // within band of the anchor
				require.NoError(t, err)
				assert.EqualValues(t, 100, r2.Status[kRef], "reference NOT re-anchored on a subsequent in-band poll")
				assert.EqualValues(t, anchored, r2.Status[kWithinEpsilon], "confirm timer not restarted")
				...
				assert.EqualValues(t, 0, r3.Status[kWithinEpsilon], "confirm timer reset on movement")
			...
			t.Run("band edge: exactly at the band suppresses, just past it resumes", func(t *testing.T) {
				...
				edge := iface.ScalingAlgorithmStatus{
					stateLastScaleUpTimestampKey: now,
					kWithinEpsilon:               now - 46_000, // confirm window elapsed
					kRef:                         float64(100),
				}
				r, err := a.ProcessMetricsPoll(ctx, cfg, edge, q.snap(100, 108)) // |108-100|=8, not > 8 -> still within band
				require.NoError(t, err)
				assert.Greater(t, r.Status.GetInt64Field(kSuppress, 0), now, "exactly at the band counts as within band and suppresses")
			...
			t.Run("dispatch dropping past the band resumes", func(t *testing.T) {
				...
				// |90-100|=10 > band 8, under a backlog still above the scale-up threshold: a real throughput drop, not a stall.
			...
			t.Run("zero dispatch under backlog is a stall, not a plateau", func(t *testing.T) {
				...
				now := time.Now().UnixMilli()
				zero := q.snap(100, 0)
				// First poll: zero rate + a backlog above the scale-up threshold must NOT anchor; growth fires to recover.
				r1, err := a.ProcessMetricsPoll(ctx, cfg, iface.ScalingAlgorithmStatus{stateLastScaleUpTimestampKey: now}, zero)
				require.NoError(t, err)
				assert.EqualValues(t, 0, r1.Status[kWithinEpsilon], "zero rate does not start confirming")
				...
				state := iface.ScalingAlgorithmStatus{stateLastScaleUpTimestampKey: now, kWithinEpsilon: now - 200_000, kRef: float64(0)}
			...
			t.Run("backlog draining clears an active lease", func(t *testing.T) {
				...
				assert.EqualValues(t, 0, r.Status[kWithinEpsilon], "confirm timer cleared")
				assert.EqualValues(t, -1, r.Status[kRef], "reference cleared")
			})

			t.Run("backlog at the scale-up threshold does not qualify -> resume", func(t *testing.T) {
				// scaleUpForBacklog := backlog > backlog_threshold (strict). At the threshold the queue does not qualify,
				// so an active verdict clears; one above it qualifies and suppresses.
				cfg := active()
				...
				require.NoError(t, err)
				assert.EqualValues(t, 0, r.Status[kSuppress], "backlog == threshold does not qualify; verdict clears")

				r2, err := a.ProcessMetricsPoll(ctx, cfg, base(), q.snap(101, 5)) // one above the threshold
				require.NoError(t, err)
				assert.Greater(t, r2.Status.GetInt64Field(kSuppress, 0), now, "one above the threshold qualifies and suppresses")
			})
	...
	t.Run("each queue's verdict uses only its own dispatch rate", func(t *testing.T) {
		now := time.Now().UnixMilli()
		// Workflow within band (suppresses on its OWN rate, ignoring activity's spike); activity
		...
		require.NoError(t, err)
		assert.Greater(t, r.Status.GetInt64Field(suppressUntilKey("workflow"), 0), now, "workflow within-band suppresses on its own verdict, regardless of activity's rate")
	...
	t.Run("maintenance (lifetime refresh) fires even while a queue is suppressed", func(t *testing.T) {
		now := time.Now().UnixMilli()
		// Confirmed within band (will suppress) and the last scale-up predates the 600s lifetime.
		state := iface.ScalingAlgorithmStatus{
	...
	t.Run("disabled detector (epsilon<=0) clears any persisted verdict", func(t *testing.T) {
		...
		state := iface.ScalingAlgorithmStatus{
			...
			dispatchRateWithinEpsilonSinceKey("activity"): now - 50_000,
			refRateKey("activity"):                        float64(5),
		}
		snap := ScalingMetricsSnapshot{Activity: &iface.QueueTypeScalingMetrics{LastBacklogCount: 100, LastProcessingRate: 5}}
		...
		assert.NotContains(t, r.Status, dispatchRateWithinEpsilonSinceKey("activity"), "within-band-since cleared when detector disabled")
	...
	t.Run("no metrics for a queue leaves its prior verdict untouched", func(t *testing.T) {
		...
		assert.EqualValues(t, now-100_000, r.Status[dispatchRateWithinEpsilonSinceKey("activity")], "within-band-since preserved")
	...
	t.Run("timing knobs fall back to defaults when unset", func(t *testing.T) {
		...
		// Within band but 1s short of the default 90s confirm window -> not suppressed; normal poll cadence.
		now := time.Now().UnixMilli()
		...
		// Within band past the default 90s window -> suppress with the default 120s lease; poll backs off to 90s.
		now = time.Now().UnixMilli()

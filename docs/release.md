...
1. **`release-auto-tag.yml`** validates, then tags:
   - `check-pin` guards that `go.mod`'s server pin is a build-tag release; on a
     pseudo-version it fails and prints the exact repin command (via
     `resolve-server-suffix.sh`). Seconds, and first, so a bad pin fails fast.
   - `ci` and `lint` run the same gates a PR runs — `ci.yaml` (go.mod sync between the
     ...
     limited to the diff.
   - `auto-tag` then computes and pushes the tag:
...
2. **`release-tag.yml`** publishes the GitHub Release, skipping the gates that already
   ran in step 1.

**The gates run before the tag exists, deliberately.** A pushed tag is immediately
resolvable by `go get` and the module proxy caches it immutably, so deleting a bad tag
does not retract it — it can only be superseded by a higher patch. Nothing is tagged
until lint and tests pass on that exact commit.
...
- A tag pushed **by a human** (not `GITHUB_TOKEN`) also triggers `release-tag.yml`
  directly. That path runs the gates itself — nothing validated the commit first — but
  the tag already exists by then, so a failure means no Release rather than no tag.

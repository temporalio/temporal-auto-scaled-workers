...
- Periodic task-queue metric polls, based on backlog and worker lifetime
  refresh, with an optional per-queue dispatch-rate-within-epsilon detector that
  suppresses scale-up when dispatch holds within the epsilon band despite backlog.
...
| `scale_up_dispatch_rate_epsilon` | number | `0` | `>= 0`, and `<= 0.10` when `> 0` | Enables the dispatch-rate-within-epsilon detector. `0` disables it. When `> 0` it is a **relative band** (a fraction of the dispatch rate): a queue's dispatch rate must stay within `epsilon * reference_rate` to count as within the band. |
| `metrics_poll_interval_ms` | integer | `60000` | `>= 10000` | Requested interval before the next periodic metric poll. |
| `scale_up_dispatch_rate_epsilon_confirm_ms` | integer | `90000` | `> metrics_poll_interval_ms` when `epsilon > 0` | A queue's dispatch rate must stay within the band under a backlog above the scale-up threshold at least this long before suppression engages. Must exceed one poll interval, else suppression fires on the second in-band poll regardless and the knob has no effect. |
| `suppress_scale_up_ms` | integer | `120000` | `> suppress_poll_interval_ms` when `epsilon > 0` | Lifetime (TTL) of the suppression lease. Each in-band poll renews it; if polling stops it self-expires after this. |
...
- `scale_up_dispatch_rate_epsilon_confirm_ms` must be `> metrics_poll_interval_ms` (at or
...
| `<queue>_dispatch_rate_within_epsilon_since_ms` | integer | When that queue's dispatch rate first entered the epsilon band under a backlog above the scale-up threshold (`0` = not within band). |
| `<queue>_dispatch_ref_rate` | number | Reference dispatch rate anchored when the band was entered (`-1` = none). |
...
`<queue>` is the task queue type (`workflow`, `activity`, `nexus`); the detector runs per type. These keys are written only when the detector is enabled
...
1. Runs the dispatch-rate-within-epsilon detector (below) for each queue type, persisting
...
for q in [workflow, activity, nexus] with metrics present:
  ...
  if candidate and q_suppressed:
    candidate = false           # suppress this queue's growth
...
## Dispatch-Rate-Within-Epsilon Detector

When `scale_up_dispatch_rate_epsilon > 0`, each metrics poll runs this detector
**per queue type**. It confirms a queue's dispatch rate staying within the epsilon band under a
backlog above the scale-up threshold (a downstream or rate-limit condition that adding workers cannot
...
The detector works for any queue type -- a non-rate-limited queue's dispatch rises as workers
...
```text
scaleUpForBacklog = backlog > scale_up_backlog_threshold
band = scale_up_dispatch_rate_epsilon * ref     # relative band
dispatchRateOutsideEpsilonBand = ref >= 0 and abs(rate - ref) > band

if not scaleUpForBacklog or dispatchRateOutsideEpsilonBand or rate <= 0:
  # no backlog above the scale-up threshold, dispatch moved off the reference, or zero throughput
  # (a stall, not holding within the band) -> resume: clear the verdict
  <q>_dispatch_rate_within_epsilon_since_ms     = 0
  <q>_dispatch_ref_rate          = -1
  <q>_suppress_scale_up_until_ms = 0
elif <q>_dispatch_rate_within_epsilon_since_ms == 0:
  # within band + backlog above the scale-up threshold -> start confirming; anchor the reference rate
  <q>_dispatch_rate_within_epsilon_since_ms = now_ms
  <q>_dispatch_ref_rate      = rate
elif now_ms - <q>_dispatch_rate_within_epsilon_since_ms >= scale_up_dispatch_rate_epsilon_confirm_ms:
  # confirmed within band under backlog -> (re)new the suppression lease
...
- The reference rate is anchored once when the band is entered and is compared
...
- Zero throughput under a backlog is treated as a stall (recover), never as a
  within-band condition to suppress.
...
- The lease is consulted by `ProcessTaskAdd` for that queue's task-adds, so the
  metrics-blind fast path obeys the same suppression lease.
